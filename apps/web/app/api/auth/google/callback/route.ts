...
export async function GET(request: Request) {
  ...
  // Exchange the Google code for tokens via Supabase
  ...
  if (phone) {
    ...
    if (walkin && walkin.id !== user.id) {
      await supabaseAdmin.from('bookings').update({ client_id: user.id }).eq('client_id', walkin.id)
      await supabaseAdmin.from('vehicles').update({ owner_client_id: user.id }).eq('owner_client_id', walkin.id)
      await supabaseAdmin.from('clients').delete().eq('id', walkin.id)
    }
  }

  // Upsert clients row (idempotent)
  ...
  return NextResponse.redirect(`${siteUrl}${roleHome(clientRow?.role ?? null)}`)
}